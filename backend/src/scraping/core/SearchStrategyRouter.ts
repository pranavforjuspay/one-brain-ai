...
import { SearchIntent, SearchStrategy, KeywordRouteMap, RouteExecutionPlan, IntelligentStrategy } from '../types/scraping.types.js';

/**
 * Intelligent router that determines the best search strategies
 * based on search intents and AI analysis
 */
export class SearchStrategyRouter {
     ...
     */
    async determineStrategies(searchIntents: SearchIntent): Promise<SearchStrategy[]> {
     ...
     */
    private analyzePlatformPreferences(searchIntents: SearchIntent): {
        web: { confidence: number; indicators: string[] };
        ios: { confidence: number; indicators: string[] };
        android: { confidence: number; indicators: string[] };
    } {
        const allText = [
     ...
     */
    private analyzeCategoryPreferences(searchIntents: SearchIntent): {
        primary: 'ecommerce' | 'fintech' | 'social' | 'productivity' | 'general';
        confidence: number;
        indicators: string[];
    } {
        const allText = [
        ...
        let bestCategory: 'ecommerce' | 'fintech' | 'social' | 'productivity' | 'general' = 'general';
        let bestScore = 0;
     ...
     */
    private analyzeSearchTypes(searchIntents: SearchIntent): {
        primary: 'keyword' | 'category' | 'app-specific';
        confidence: number;
        reasoning: string;
    } {
     ...
     */
    private calculatePriority(platformConfidence: number, categoryConfidence: number, searchTypeConfidence: number): number {
     ...
     */
    analyzeKeywordRoutes(keywords: string[]): IntelligentStrategy {
        const routingStartTime = Date.now();
        const routingId = Math.random().toString(36).substring(7);
        ...
        const keywordRouteMap: KeywordRouteMap = {
        ...
        const executionPlans: RouteExecutionPlan[] = [];
        ...
        const optimizedPhases = executionPlans.map(plan =>
            plan.route === 'apps' ? `apps_${plan.platforms.join('_')}` : plan.route
        );
        ...
        const intelligentStrategy: IntelligentStrategy = {
            keywordRouteMap,
            executionPlans,
            optimizedPhases,
            estimatedDuration,
     ...
     */
    private isAppKeyword(keyword: string): boolean {
        const appKeywords = [
     ...
     */
    private isScreenKeyword(keyword: string): boolean {
     ...
     */
    private isFlowKeyword(keyword: string): boolean {
        const flowKeywords = [
     ...
     */
    private isHybridKeyword(keyword: string): boolean {
     ...
     */
    private isUIElementKeyword(keyword: string): boolean {
        const uiKeywords = [
     ...
     */
    private selectKeywords(searchIntents: SearchIntent, platform: 'web' | 'ios' | 'android' | 'all'): string[] {
