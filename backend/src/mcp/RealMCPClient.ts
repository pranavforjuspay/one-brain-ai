...
export class RealMCPClient extends EventEmitter {
    ...
    private async listTools(): Promise<void> {
        ...
        try {
            ...
            if (response.tools && Array.isArray(response.tools)) {
                this.availableTools = response.tools.map((tool: any) => tool.name);
    ...
    /**
     * Use an MCP tool with configurable timeout
     */
    async useTool(toolName: string, args: any, timeoutMs: number = 90000): Promise<any> {
        if (!this.isConnected) {
            await this.connect();
        }
    ...
    /**
     * Send a request to the MCP server with configurable timeout
     */
    private async sendRequest(method: string, params: any, timeoutMs: number = 60000): Promise<any> {
        return new Promise((resolve, reject) => {
            ...
            const timeout = setTimeout(() => {
                if (this.pendingRequests.has(id)) {
                    ...
                    reject(new Error(`Request ${id} (${method}) timed out after ${timeoutMs / 1000} seconds`));
                }
            }, timeoutMs);
            ...
            console.log(`[REAL_MCP] Sending request ${id}:`, { method, paramsKeys: Object.keys(params), timeoutMs });
...
let browserSessionActive: boolean = false;
let browserSessionLock: Promise<void> | null = null;
 ...
 */
export async function waitForBrowserSession(): Promise<void> {
 ...
 */
export async function useRealMCPTool(serverName: string, toolName: string, args: any): Promise<any> {
